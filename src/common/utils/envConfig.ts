...
export const env = cleanEnv(process.env, {
  ...
  DB_REPLICA_CONNECTION_STRING: str({ devDefault: testOnly('test') }),
  DB_LOCAL_CONNECTION_STRING: str({ devDefault: testOnly('test') }),
  ...
  ALLOCATOR_BASE_REPORT_URL: str({ devDefault: testOnly('http://localhost:8080') }),
